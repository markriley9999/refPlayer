...
ffmpeg -y  -i %CONTENT% -vf "scale=-1:1080, crop=1920:1080" -c:v libx264 -r 25 -profile:v main -preset fast -c:a aac -b:a 128k -ar 48000 -ac 2 -movflags +faststart -t %DURATION% %DIR%\source.mp4


REM --- Split out Video and Audio ---

ffmpeg -y  -i %DIR%\source.mp4 -map_metadata -1 -map 0:0 -c:v copy -an %DIR%\video.mp4
ffmpeg -y -i %DIR%\source.mp4 -map_metadata -1 -c:a copy -vn %DIR%\audio.m4a
...
ffmpeg -y  -i %DIR%\video.mp4 -vf "scale=512:288" -c:v libx264 -b:v 438k -x264opts keyint=96:min-keyint=96:no-scenecut -profile:v main -preset medium -movflags +faststart %DIR%\valigned_5.mp4
...
ffmpeg -y  -i %DIR%\valigned_5.mp4 -map_metadata -1 -map 0:0 -c:v copy -an %DIR%\v5.mp4
...
ffmpeg -y -i %DIR%\v5.mp4 -i %DIR%\audio.m4a -c copy %DIR%\checkpt_1.mp4
ffmpeg -y -i %DIR%\v1.mp4 -i %DIR%\audio.m4a -c copy %DIR%\checkpt_2.mp4
...
ffprobe -show_frames -print_format compact %DIR%\v5.mp4 > %DIR%\frames_5.txt
